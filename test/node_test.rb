...
describe RichText::Node do
  ...
  describe '#<<' do
    ...
    it 'rejects other objects' do
      assert_raises(TypeError) { node << 'not a node' }
    end
  end
  ...
  describe '#equal?' do
    it 'returns false when two nodes do not share the same attributes' do
      node_a = subject.new name: 'a'
      node_b = subject.new name: 'b'
    ...
    it 'returns false when the child count is different' do
      node_a = subject.new name: 'n'
      node_b = subject.new name: 'n'
      node_b << subject.new
